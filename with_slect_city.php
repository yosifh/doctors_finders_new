...
<body>
  ...
  <div class="main-wrapper">
    ...
    $sql = "SELECT cityes.city_name, spec.name FROM doctors
        JOIN cityes ON (doctors.city_id=cityes.id) 
        JOIN spec ON (doctors.spec_id=spec.id) 
        WHERE spec_id='" . $spec . "' AND city_id='" . $city . "'";
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            // عرض اسم الاختصاص والمدينة
            echo "<h1>  أفضل الأطباء أصحاب أختصاص $spec_name في  $city_name </h1>";

            // توقف عند الطبيب الأول
            break;
        }
    } else {
        // رسالة إذا لم يتم العثور على أي بيانات
        echo "<h1>لا يوجد أطباء بهذا الاختصاص في هذه المدينة</h1>";
    }
...
?>
            ...
            <p>اكتشف أفضل الأطباء في المدينة الأقرب إليك.</p>
          </div>
            ...
            <div class="row row-grid">
              ...
              if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                  ...
                  if (!in_array($full_name, $doctorsDisplayed)) {
                    $doctorsDisplayed[] = $full_name;
